...
public class LoadBalancer {
    ...
    public String post(String message, String path) throws Exception {
        ...
        Request request = new Request.Builder().url(baseUrl + ports[numberOfServers] + path).post(body).build();
    ...
    public String get(String path) throws Exception {
         ...
         * .url(baseUrl+ports[numberOfServers]+path) .get() .build(); Response response
         * = httpClient.newCall(request).execute();
         * System.out.println("get enviado a "+baseUrl+ports[numberOfServers]+path);
         * return response.body().string();
         */
        StringBuilder result = new StringBuilder();
        URL url = new URL(baseUrl+ports[numberOfServers]+path);
